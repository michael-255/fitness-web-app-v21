...
const isDisabled = computed(
    () => $q.loading.isActive || selectedStore.workout.status.includes(StatusEnum.LOCKED),
)
const deactivated = computedStatusToggle(selectedStore.workout.status, StatusEnum.DEACTIVATED)
const favorited = computedStatusToggle(selectedStore.workout.status, StatusEnum.FAVORITED)

const displayDate = computed(
    () => date.formatDate(selectedStore.workout.createdAt, displayDateFormat) ?? '-',
)
const dateTimePicker = ref(date.formatDate(selectedStore.workout.createdAt, pickerDateFormat))

watch(
    () => selectedStore.workout.createdAt,
    (newTimestamp) => {
...
watch(dateTimePicker, () => {
    ...
    if (newTimestamp && !isNaN(newTimestamp)) {
        ...
        selectedStore.workout.createdAt = newTimestamp
    }
})
</script>
...
<template>
    <q-list padding>
        ...
        <BaseFormItem label="Created Date" description="Date and time this record was created.">
            ...
            <q-item-label class="q-gutter-xs">
                ...
                <q-btn
                    ...
                    label="Now"
                    color="positive"
                    @click="selectedStore.workout.createdAt = Date.now()"
                />
            </q-item-label>
        </BaseFormItem>
        ...
        <BaseFormItem label="Name" description="Customizable name for this record.">
            <q-item-label>
                <q-input
                    v-model="selectedStore.workout.name"
                    @blur="selectedStore.workout.name = selectedStore.workout.name?.trim()"
                    :rules="[
                        (val: string) =>
                ...
                >
                    <template v-slot:append>
                        <q-icon
                            v-if="selectedStore.workout.name !== ''"
                            @click="selectedStore.workout.name = ''"
        ...
        <BaseFormItem label="Description" description="Optional description for this record.">
            <q-item-label>
                <q-input
                    v-model="selectedStore.workout.desc"
                    @blur="selectedStore.workout.desc = selectedStore.workout.desc?.trim()"
                    :rules="[
                        (val: string) =>
                ...
                >
                    <template v-slot:append>
                        <q-icon
                            v-if="selectedStore.workout.desc !== ''"
                            @click="selectedStore.workout.desc = ''"
                            class="cursor-pointer"
                            :name="cancelIcon"
                        />
                    </template>
        ...
        <q-item v-show="!selectedStore.isWorkoutValid">
            <q-item-section>
