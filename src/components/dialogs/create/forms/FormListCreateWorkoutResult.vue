...
const exampleService = WorkoutService()
...
const displayDate = computed(
    () => date.formatDate(selectedStore.workoutResult.createdAt, displayDateFormat) ?? '-',
)
const dateTimePicker = ref(date.formatDate(selectedStore.workoutResult.createdAt, pickerDateFormat))
...
onMounted(async () => {
    try {
        ...
        if (!workoutIdMatch) {
            selectedStore.workoutResult.workoutId = undefined! // If no options, or id is invalid
        }
    } catch (error) {
        log.error('Error loading Parent Workout Id options', error as Error)
    }
})

watch(
    () => selectedStore.workoutResult.createdAt,
    (newTimestamp) => {
...
watch(dateTimePicker, () => {
    ...
    if (newTimestamp && !isNaN(newTimestamp)) {
        ...
        selectedStore.workoutResult.createdAt = newTimestamp
    }
})
</script>
    ...
    <q-list padding>
        ...
        >
            <q-item-label caption>
                {{ selectedStore.workoutResult?.id ?? '-' }}
            </q-item-label>
        </BaseFormItem>

        <BaseFormItem
            label="Parent Workout Id"
            description="Id of the parent record that this child record is associated with."
        >
            <q-item-label caption>
                <q-select
                    v-model="selectedStore.workoutResult.workoutId"
        ...
        <BaseFormItem label="Created Date" description="Date and time this record was created.">
            ...
            <q-item-label class="q-gutter-xs">
                ...
                <q-btn
                    ...
                    label="Now"
                    color="positive"
                    @click="selectedStore.workoutResult.createdAt = Date.now()"
                />
            </q-item-label>
        </BaseFormItem>
        ...
        <BaseFormItem label="Note" description="Optional description for this record.">
            <q-item-label>
                <q-input
                    v-model="selectedStore.workoutResult.note"
                    @blur="
                        selectedStore.workoutResult.note = selectedStore.workoutResult.note?.trim()
                    "
                    :rules="[
                        (val: string) =>
                ...
                >
                    <template v-slot:append>
                        <q-icon
                            v-if="selectedStore.workoutResult.note !== ''"
                            @click="selectedStore.workoutResult.note = ''"
        ...
        <q-item v-show="!selectedStore.isWorkoutResultValid">
            <q-item-section>
