...
const isDisabled = computed(
    () => $q.loading.isActive || selectedStore.exercise.status.includes(StatusEnum.LOCKED),
)
const deactivated = computedStatusToggle(selectedStore.exercise.status, StatusEnum.DEACTIVATED)
const favorited = computedStatusToggle(selectedStore.exercise.status, StatusEnum.FAVORITED)

const displayDate = computed(
    () => date.formatDate(selectedStore.exercise.createdAt, displayDateFormat) ?? '-',
)
const dateTimePicker = ref(date.formatDate(selectedStore.exercise.createdAt, pickerDateFormat))

watch(
    () => selectedStore.exercise.createdAt,
    (newTimestamp) => {
...
watch(dateTimePicker, () => {
    ...
    if (newTimestamp && !isNaN(newTimestamp)) {
        ...
        selectedStore.exercise.createdAt = newTimestamp
    }
})
</script>
...
<template>
    <q-list padding>
        ...
        <BaseFormItem label="Created Date" description="Date and time this record was created.">
            ...
            <q-item-label class="q-gutter-xs">
                ...
                <q-btn
                    ...
                    label="Now"
                    color="positive"
                    @click="selectedStore.exercise.createdAt = Date.now()"
                />
            </q-item-label>
        </BaseFormItem>
        ...
        <BaseFormItem label="Name" description="Customizable name for this record.">
            <q-item-label>
                <q-input
                    v-model="selectedStore.exercise.name"
                    @blur="selectedStore.exercise.name = selectedStore.exercise.name?.trim()"
                    :rules="[
                        (val: string) =>
                ...
                >
                    <template v-slot:append>
                        <q-icon
                            v-if="selectedStore.exercise.name !== ''"
                            @click="selectedStore.exercise.name = ''"
        ...
        <BaseFormItem label="Description" description="Optional description for this record.">
            <q-item-label>
                <q-input
                    v-model="selectedStore.exercise.desc"
                    @blur="selectedStore.exercise.desc = selectedStore.exercise.desc?.trim()"
                    :rules="[
                        (val: string) =>
                ...
                >
                    <template v-slot:append>
                        <q-icon
                            v-if="selectedStore.exercise.desc !== ''"
                            @click="selectedStore.exercise.desc = ''"
                            class="cursor-pointer"
                            :name="cancelIcon"
                        />
                    </template>
        ...
        <q-item v-show="!selectedStore.isExerciseValid">
            <q-item-section>
